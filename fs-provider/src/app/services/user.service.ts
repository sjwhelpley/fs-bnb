...
export class UserService {
  ...
  register(firstName, lastName, cellPhone, email, password) {
    ...
    return new Promise((resolve, reject) => {
      this.http.post('http://localhost:5000/api/auth/register/provider', JSON.stringify(newUser), httpOptions).subscribe((response: any) => {
        localStorage.setItem("userId", response.user.id);
        localStorage.setItem("jwt", response.jwt);
        resolve(response);
      });
  ...
  getById(id) {
    return new Promise((resolve, reject) => {
      ...
      this.http.get(`http://localhost:5000/api/provider/${id}`, httpOptions).subscribe((response) => {
        resolve(response);
      });
    });
  }
